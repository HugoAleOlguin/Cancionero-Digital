...
def evaluate_song(hymn):
    ...
    t_words = [w for w in norm(title).split() if len(w) > 2 and w not in ["que", "del", "las", "los", "por", "para", "con", "una", "uno", "mas"]]
    ...
    for c in candidates:
        ...
        has_artist = ("acuna" in v_title_norm or "acuna" in v_channel_norm)
    ...
    return {
        "id": hymn["id"],
        "title": title,
        "author": hymn.get("author", "Trio Acuña"),
        "status": status,
        "best_match": best,
...
def main():
    ...
    acuna_hymns = [h for h in catalog if ('acuña' in h.get('author', '').lower() or 'acuna' in h.get('author', '').lower()) and not h.get("isDeleted", False)]
    
    print(f"Iniciando búsqueda y verificación en YouTube para {len(acuna_hymns)} alabanzas de Trío Acuña...\n")
    
    results = []
    for idx, h in enumerate(acuna_hymns, 1):
        print(f"[{idx:2d}/{len(acuna_hymns):2d}] Buscando #{h['id']} '{h['title']}'...")
        res = evaluate_song(h)
        results.append(res)
        best = res["best_match"]
        if best and res["status"] in ["EXACT_MATCH", "HIGH_CONFIDENCE"]:
            print(f"   -> [{res['status']}] '{best['videoTitle']}' ({best['url']}) [Sc: {best['score']}]")
        elif best:
    ...
    out_path = "data/youtube_acuna_results.json"
