...
trigales = [h for h in cat if 'trigales' in h.get('author', '').lower() and not h.get('isDeleted', False)]

print(f"Total Alabanzas Conjunto Trigales: {len(trigales)}")
for idx, h in enumerate(trigales, 1):
    link = h.get('link', '')
